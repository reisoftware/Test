...
function Class:on_draw_rendering()
	self:set_shape_rendering(require"app.LaserRuler.shape".draw{spt=self:get_spt(),ept=self:get_ept(),sections=self.Sections,require"sys.geometry".Color:new(self.Color),circle=self.Circle,alignment=self.Alignment,mode="rendering"});
end
