...
local group_members_;
local contact_config_data_;
local dlg_ = nil
local data_ = nil
...
function pop()
	...
	local function init_list()
		...
		list_groups_[1] = nil
		contact_config_data_ = tree_config_.get_contacts()
		if not contact_config_data_ then iup.Message("Warning","No contacts !") return end 
		for i=1,#contact_config_data_ do 
			list_groups_.APPENDITEM = contact_config_data_[i].Title
		end 
