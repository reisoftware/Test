...
local dlg_ = iup.vbox{
	...
	-- size = "800x800";
	iup.vbox{
		iup.hbox{update_,add_btn_,del_btn_,open_view_btn_,open_file_btn_};
...
function pop(t)
	...
	local function get_dat_items()
		local all_ = require"sys.mgr".get_class_all(require"app.Appendix.File".Class);
	...
	local function init_name_mat()
		...
		require'sys.api.iup.matrix'.init_list{mat=name_mat_,fields=name_mat_fields_,dat=dat,minlin=50,sortf=function(k) return dat[k].Time end};
	end
	...
	local function on_add()
		...
		require'sys.net.file'.send{
			...
			cbf=function() 
				...
				local item = require'app.Appendix.File'.Class:new{Name=name,hid=hid}
	...
	local function get_selection_lin_item()
		...
		-- local all = require"sys.mgr".get_class_all(require"app.Appendix.File".Class);
	...
	init();
	-- require'sys.api.iup.key'.register_k_any{dlg=dlg_,[iup.K_CR]=on_look};
