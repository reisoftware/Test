...
function wireframe(sc)
	...
	local ents = require"sys.mgr".get_scene_selection();
	if not ents then return end
	local run = require"sys.progress".create{title="Wireframe",count=require"sys.table".count(ents),time=1};
	for k,v in pairs(ents) do
		v = require"sys.mgr".get_table(k,v);
...
function rendering(sc)
	...
	local ents = require"sys.mgr".get_scene_selection();
	if not ents then return end
	local run = require"sys.progress".create{title="Rendering",count=require"sys.table".count(ents),time=1};
	for k,v in pairs(ents) do
		v = require"sys.mgr".get_table(k,v);
